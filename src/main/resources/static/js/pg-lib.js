...
const MSG_NOT_DESCRIPTION = "¡Debes escribir una descripción!";
const MSG_NOT_WORD = "¡Debes escribir una palabra!";
...
function generarDefinicionLarga() {
	var texto= $("#texto").val();
	if (texto.length==0){
		ModalNotif(MSG_NOT_DESCRIPTION)
		return 0;
	}
...
/**
 * Agrega a la lista si la palabra está aceptada o no
 * @param {*} data 
 * @param {*} palabra 
 */
...
/**
 * Esta función permite vaciar la informacion mostrada del automata
 * generado anteriormente para evitar apilar información innecesaria
 */
function vaciarAutomata(){
    //Vaciamos los resultados anteriores
    // TODO: maybe bug ? por si generamos un nuevo automata
...
/**
 * GUI : permite lanzar un modal con un mensaje y un retardo
 * de tiempo en el que debe cerrarse
...
/**
 * Permite manejar el evento Click de los botones
 * 'Usar ejemplo' en los distintos tabs
...
function UseExample(e) {
	...
	switch (element.attr("id")) {
		case "useExampleNotDeterministic":
			...
			break;
		case "useExampleSimpleDeterministic":
...
function Main() {
	...
	$("#useExampleSimpleDeterministic").on('click', UseExample)
