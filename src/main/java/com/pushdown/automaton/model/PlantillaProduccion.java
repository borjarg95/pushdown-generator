package com.pushdown.automaton.model;
...
public class PlantillaProduccion{
	 ...
	 * @throws Exception 
	 */
	PlantillaProduccion(String estado1, Character simboloPila, String estado2) throws DatosEntradaErroneosException {
		if (!GenericValidator.isBlankOrNull(estado1)){
			if (!GenericValidator.isBlankOrNull(estado2)){ //Correctamente formado y no generativo/Axioma
				...
				throw new DatosEntradaErroneosException("La producción ["+estado1+simboloPila+estado2+"], no está bien informada");
			}
		} else {
